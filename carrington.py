...
def test_field_open(f, coord_list, fg_b = None, max_iterations = 15000, dx = R_EARTH / 50, trace_method = 'integrateB', coord_in='cartesian' ): 
    ...
    if type(f) == str: #convert filename to .vlsv object. Using recognizably different inputs (such as filenames instead of vlsv objects) enables @sidecar
...
#        indgt = np.where(z >= 0)
#        indlt = np.where(z < 0)
#        xgt,ygt,zgt,xxgt,yygt,zzgt = geopack.trace(x[indgt],y[indgt],z[indgt],dir=-1,rlim=100,r0=.99999,parmod=2,exname='t89',inname='igrf',maxloop=10000)
#        xlt,ygt,zlt,xxlt,yylt,zzlt = geopack.trace(x[indlt],y[indlt],z[indlt],dir=-1,rlim=100,r0=.99999,parmod=2,exname='t89',inname='igrf',maxloop=10000)
...
def volume_shell(data, x, y, z, threshold, delta, step = 1):
    ...
    '''
    rcyl = (y**2 + z**2)**0.5
    rcyls = np.arange(np.nanmin(rcyl), np.nanmax(rcyl), step)
    x_shell = []; rcyl_shell = []
    for rcyl_i in rcyls:
        ind1, = np.nonzero( (np.abs(data - threshold) < delta) & (rcyl>=rcyl_i) & (rcyl<(rcyl_i+step)))
        #ind1, = np.nonzero( ( (data - threshold) > 0) & (rcyl>=rcyl_i) & (rcyl<(rcyl_i+step)))   # NOTE: this deprecates delta
        if ind1.size > 0:
            x_shell.append(np.nanmax(x[ind1]))
            rcyl_shell.append(rcyl_i + step/2)
    return np.array(x_shell), np.array(rcyl_shell)
    #return np.abs(data - threshold) < delta
...
def magnetopause(f, threshold = 1, delta = 0.05):
    ''' define magnetopause as where modified plasma beta equals 1
        (Brenner et al, 2021)
    '''
    vg_x, vg_y, vg_z = get_all_cell_coordinates(f)
    x = vg_x / R_EARTH; y = vg_y / R_EARTH; z = vg_z / R_EARTH
...
def fit_bow_shock(f, run = '', root_dir = '', fileIndex = '', threshold = 2, delta = 0.05, plot = True, method = 'n', file_suffix = ''):
    ...
    if plot:
        ...
        filename = '{}bow_shock_pos_data_{}_{}{}.png'.format(save_dir, method, str(fileIndex).zfill(5), file_suffix)
        plt.savefig(filename)
        plt.close()
    return f_shue_fit, theta_bs, r_bs


def fit_magnetopause(f, run = '', root_dir = '', fileIndex = '', threshold = 1, delta = 0.05, plot = True, file_suffix = ''):
    #vg_x, vg_y, vg_z = bow_shock(f, threshold = threshold, delta = delta)
    #vg_rcyl = (vg_y**2 + vg_z**2)**0.5
    ...
    if plot:
        save_dir = '{}{}/magnetopause_pos/{}/'.format(root_dir, run.upper(), str(fileIndex).zfill(7))         #CUSTOM
        rplot = f_shue_fit(theta_mp)
        title = r'{} Magnetopause position, time={}'.format(run.upper(), fileIndex)
        plt.scatter(vg_x, vg_rcyl, color = 'red')
        plt.xlabel(r'x [$r_E$]')
        plt.ylabel(r'$\sqrt{y^2 + z^2}$ [$r_E$]')
        plt.title(title)
        plt.plot(rplot * np.cos(theta_mp), rplot * np.sin(theta_mp))
        filename = '{}magnetopause_pos_data_{}{}.png'.format(save_dir, str(fileIndex).zfill(5), file_suffix)
        plt.savefig(filename)
        plt.close()
    return f_shue_fit, theta_mp, r_mp


def inner_radius(f):
    #inner boundary of simulation [R_E]
    vg_x, vg_y, vg_z = get_all_cell_coordinates(f)
    x = vg_x / R_EARTH; y = vg_y / R_EARTH; z = vg_z / R_EARTH
    r = np.sqrt(x**2 + y**2 + z**2)
    rho = f.read_variable('proton/vg_rho')
    return np.nanmax(r[np.where(rho == 0)[0]])
...
class ParamObj:
    ...
    def __init__(self, run, f, coord_list, coord_list_traced = None, fileIndex = None, file_suffix = '', plot_data=False, make_plot_data = False, coord_in = 'spherical'):
        ...
        self.file = get_vlsvfile_fullpath(self.run, self.fileIndex)
...
#            os.system('mkdir -p {}'.format(filedir))                  # need a function to just compute the save dir from the path using split('/') and indexing
    def make_lonlat_plot(self, phi, theta, plot_variable_2d, xlabel='longitude [deg.]', ylabel='latitude [deg.]', title = '', cbar = True, cbar_label='[SI]', extent = [-180, 180, -90, 90], 
        ...
        # PLOT : azimuthal projection of fluxes
        fig, ax = plt.subplots()
        ...
        im = ax.pcolormesh(phi*180/np.pi, lat*180/np.pi, plot_variable_2d,
    ...
    def make_polar_plot(self, phi, theta, plot_variable_2d, cbar = True, cbar_label='[SI]', title = '',
                         theta_min=0, theta_max = 30, vmin = None, vmax = None, cmap = plt.cm.get_cmap('plasma'), path = './test.png', pltclose =  True):
        fig = plt.figure()
        ...
        if np.nanmin(theta_plot) > 90:
            theta_plot = 180 - theta_plot
        im = ax1.pcolormesh(phi_plot, np.abs(theta_plot), plot_variable_2d,
        ...
        mkdir_path(path)
        plt.savefig(path)
        plt.sca(ax1)   # set the current axis to the main plot 
...
class pFluxObj(ParamObj):
    ...
    def make_plot_data(self, plot_data=False, plots = [0, 1, 2, 3, 4, 5, 6, 7, 8], **kwargs):
        ...
        for i, key in enumerate(dct):
            ...
            title = '{} {} eV DNF, time={}'.format(self.run.upper(), int(proton_energy), self.fileIndex)
            ...
            self.data_label_list.append( 'proton_DNF_{}_eV'.format(str(int(proton_energy)).zfill(5)) )
            save_dir = '{}{}/proton_DNF/{}/'.format(self.root_dir, self.run.upper(), str(self.fileIndex).zfill(7))
            if plot_data:
                if i in plots:
                    # PLOT a: square long-lat projection of fluxes
                    ...
                    # PLOT b (this axis object is returned ): azimuthal projection of fluxes
                    ...
                    self.make_polar_plot(phi, theta, np.log10(plot_variable_2d),
                                         cbar_label=r'log$_{10}$(DNF) [cm$^{-2} s^{-1} sr^{-1} eV^{-1}$]', title = title,
                                         ...
                                         vmin = np.log10(100000*np.exp(-proton_energy / 5000)) - 8, 
                                         vmax = np.log10(100000*np.exp(-proton_energy / 5000)) - 1, 
                                         cmap = plt.cm.get_cmap('plasma'), path = filename, **kwargs  )
        if plot_data:
            return plt.gca()
...
class dBdtObj(ParamObj):
    def __init__(self, run, f, coord_list, **kwargs):
        self.dct = None
        ParamObj.__init__(self, run, f, coord_list, **kwargs)
    def make_plot_data(self, plot_Data=False, plots = [0,1,2], **kwargs):
            ...
            if plot_data:
                if i in plots:
                    ...
                    self.make_lonlat_plot(phi, theta, plot_variable_2d, xlabel='longitude [deg.]', ylabel='latitude [deg.]', title = title, cbar_label=r'dB$_{}$/dt [T/sec]'.format(subscripts[i]),
                                          extent = [-180, 180, -90, 90], vmin = np.nanmin(plot_variable_2d), vmax = np.nanmax(plot_variable_2d), cmap = plt.cm.get_cmap('plasma'), path = filename, **kwargs) 
        if plot_data:
            return plt.gca()
...
class FieldOpenObj(ParamObj):
    def __init__(self, run, f, coord_list, fg_b = None, max_iterations = 15000, dx = R_EARTH / 50, **kwargs):
        ...
        ParamObj.__init__(self, run, f, coord_list, **kwargs) 
    def make_plot_data(self, plot_data=False, plots = [0, 1, 2], **kwargs):
        plot_variable_2d = test_field_open(self.file, self.coord_list, fg_b = self.fg_b, max_iterations = self.max_iterations, dx = self.dx, trace_method = 'integrateB', coord_in = 'cartesian' )
        ...
        if plot_data:
            if 0 in plots:
                ...
                #self.make_polar_plot(phi[(nlat-delta_lat_ind-1):,:], theta[(nlat-delta_lat_ind-1):,:], plot_variable_2d[(nlat-delta_lat_ind-1):,:],
                                    ...
                                    cbar = False, title = title, vmin = np.nanmin(plot_variable_2d), vmax = np.nanmax(plot_variable_2d), cmap = plt.cm.get_cmap('Wistia'), path = filename, **kwargs)
            if 1 in plots:
                ...
                self.make_lonlat_plot(phi, theta, plot_variable_2d, xlabel='longitude [deg.]', ylabel='latitude [deg.]', title = title, cbar = False,
                                     ...
                                     vmin = np.nanmin(plot_variable_2d), vmax = np.nanmax(plot_variable_2d), cmap = plt.cm.get_cmap('Wistia'), path = filename, **kwargs)
            if 2 in plots:
                ...
                plt.quiver(x2d[0::dq,0::dq]/R_EARTH, z2d[0::dq,0::dq]/R_EARTH, 
                           2*self.fg_b[0::dq,yeq0_ind,0::dq,0]/B[0::dq,yeq0_ind,0::dq], 2*self.fg_b[0::dq,yeq0_ind,0::dq,2]/B[0::dq,yeq0_ind,0::dq],**kwargs )
                # plot shue model
                step = np.pi / 200   
                theta_shue = np.arange( -(np.pi-step), np.pi-step, step)
                r_shue, r_0_shue, alpha_shue = f_shue(theta_shue, run = self.run)
                plt.plot( r_shue * np.cos(theta_shue), r_shue * np.sin(theta_shue), color = 'black', **kwargs )
                plt.plot(  np.cos(theta_shue), np.sin(theta_shue), color = 'blue', linestyle = 'dotted', **kwargs )        # Earth has radius = 1 on this plot
                plt.title(title)
                plt.xlabel(r'x [$R_E$]')
                plt.ylabel(r'z [$R_E$]')
                plt.xlim([self.f.read_parameter('xmin') / R_EARTH, self.f.read_parameter('xmax') / R_EARTH])
                plt.ylim([self.f.read_parameter('zmin') / R_EARTH, self.f.read_parameter('zmax') / R_EARTH])
                save_dir = '{}{}/field_open/{}/'.format(self.root_dir, self.run.upper(), str(self.fileIndex).zfill(7))
                path = '{}field_open_closed_lines_{}{}.png'.format(save_dir, str(self.fileIndex).zfill(5), self.file_suffix)
                mkdir_path(path)
                plt.savefig(path)
                plt.close()
            return plt.gca()
...
class FAC_Obj(ParamObj):
    def __init__(self, run, f, coord_list, fg_b = None, **kwargs):
        if (fg_b is None):
            fg_b = f.read_variable('fg_b')     
        self.fg_b = fg_b
        self.cellsize_xyz = [ (f.read_parameter('xmax') - f.read_parameter('xmin')) / fg_b.shape[0],
                            (f.read_parameter('ymax') - f.read_parameter('ymin')) / fg_b.shape[1],
                            (f.read_parameter('zmax') - f.read_parameter('zmin')) / fg_b.shape[2] ]
        ParamObj.__init__(self, run, f, coord_list, **kwargs) 
    #@sidecar
    def calc_J(self, coord_list = None, fg_b = None, style = 'jrad', b_scaled = True):
    #use self.fg_b, self.coord_list, self.cellsize_xyz
    #note: fg_b is treated as a keyword parameter here in order to enable @sidecar to save data
        if fg_b is None:
            fg_b = self.fg_b
        if coord_list is None:
            coord_list = self.coord_list
        #test for the maximum extent of coord_list (rmin)
        x_out = deepcopy(self.coord_list_traced[0]); y_out = deepcopy(self.coord_list_traced[1]); z_out = deepcopy(self.coord_list_traced[2])
        rmin = np.sqrt(np.nanmin(x_out**2 + y_out**2 + z_out**2))
        # reduce array size to speed up curl calculation (don't need to calculate @ r > rmin )
        nx,ny,nz,dummy = fg_b.shape
        xmin = self.f.read_parameter('xmin'); xmax = self.f.read_parameter('xmax')
        ymin = self.f.read_parameter('ymin'); ymax = self.f.read_parameter('ymax')
        zmin = self.f.read_parameter('zmin'); zmax = self.f.read_parameter('zmax')
        dx = self.cellsize_xyz[0]; dy = self.cellsize_xyz[1]; dz = self.cellsize_xyz[2]
        ixa = int(-(rmin + xmin) / dx)-3; ixb = int((rmin - xmin) / dx) + 3  # pad w/ cells to compute gradient everywhere
        iya = int(-(rmin + ymin) / dy)-3; iyb = int((rmin - ymin) / dy) + 3
        iza = int(-(rmin + zmin) / dz)-3; izb = int((rmin - zmin) / dz) + 3
        ixa = np.max([0,ixa]) ; ixb = np.min([ixb, nx-1])
        iya = np.max([0,iya]) ; iyb = np.min([iyb, ny-1])
        iza = np.max([0,iza]) ; izb = np.min([izb, nz-1])
        fg_b_calc = fg_b[ixa:ixb, iya:iyb, iza:izb, :]
        xmin_intp = xmin + ixa*dx ; xmax_intp = xmin + (ixb+1)*dx    # reducing array size affect interpolation
        ymin_intp = ymin + iya*dy ; ymax_intp = ymin + (iyb+1)*dy  
        zmin_intp = zmin + iza*dz ; zmax_intp = zmin + (izb+1)*dz  
        # calculate the current
        cellsize_xyz = self.cellsize_xyz
        #self.j = (1 / mu_0) * numcurl3d(fg_b_calc, CELLSIZE_XYZ)
        if fg_b_calc.size < 2e8:
            j = (1 / mu_0) * numcurl3d(fg_b_calc, self.cellsize_xyz)
        else:
        # if array is too large: break the calculation into bits to take up less memory (probably a bit slower)
            j = fg_b_calc*0
            nx2 = int(fg_b_calc.shape[0]/2)
            ny2 = int(fg_b_calc.shape[1]/2)
            nz2 = int(fg_b_calc.shape[2]/2)
            j[0:nx2,0:ny2,0:nz2,:] = (1 / mu_0) * numcurl3d(fg_b_calc[0:nx2,0:ny2,0:nz2,:], self.cellsize_xyz)
            j[nx2:,0:ny2,0:nz2,:] = (1 / mu_0) * numcurl3d(fg_b_calc[nx2:,0:ny2,0:nz2,:], self.cellsize_xyz)
            j[0:nx2,ny2:,0:nz2,:] = (1 / mu_0) * numcurl3d(fg_b_calc[0:nx2,ny2:,0:nz2,:], self.cellsize_xyz)
            j[0:nx2,0:ny2,nz2:,:] = (1 / mu_0) * numcurl3d(fg_b_calc[0:nx2,0:ny2,nz2:,:], self.cellsize_xyz)
            j[0:nx2,ny2:,nz2:,:] = (1 / mu_0) * numcurl3d(fg_b_calc[0:nx2,ny2:,nz2:,:], self.cellsize_xyz)
            j[nx2:,0:ny2,nz2:,:] = (1 / mu_0) * numcurl3d(fg_b_calc[nx2:,0:ny2,nz2:,:], self.cellsize_xyz)
            j[nx2:,ny2:,0:nz2,:] = (1 / mu_0) * numcurl3d(fg_b_calc[nx2:,ny2:,0:nz2,:], self.cellsize_xyz)
            j[nx2:,ny2:,nz2:,:] = (1 / mu_0) * numcurl3d(fg_b_calc[nx2:,ny2:,nz2:,:], self.cellsize_xyz)
        ...
        interpolators_J = interpolator_list_3d(self.f, j, bounds_error = False, fill_value = np.nan, 
                                                xmin = xmin_intp, xmax = xmax_intp,
                                                ymin = ymin_intp, ymax = ymax_intp,
                                                zmin = zmin_intp, zmax = zmax_intp)
        interpolators_B = interpolator_list_3d(self.f, fg_b_calc, bounds_error = False, fill_value = np.nan,
                                                xmin = xmin_intp, xmax = xmax_intp,
                                                ymin = ymin_intp, ymax = ymax_intp,
                                                zmin = zmin_intp, zmax = zmax_intp)
        ...
        plot_variable_2d = plot_variable_2d.reshape(self.shape)
        return plot_variable_2d
    def make_plot_data(self, plot_data=False, style = 'jpar', b_scaled = True, plots = [0,1], **kwargs): 
        plot_variable_2d = self.calc_J(coord_list = self.coord_list, fg_b = self.fg_b, style = style, b_scaled = b_scaled)
        if b_scaled == True:
            scaled_string = 'scaled'
            cbar_string = '(B_0 / B)'
        else:
        ...
        if plot_data:
            ...
            r, theta, phi = self.coord_list_sphere
            if 0 in plots:
                #PLOT 0: rectangular coordinate system
                ...
                self.make_lonlat_plot(phi, theta, plot_variable_2d, xlabel='longitude [deg.]', ylabel='latitude [deg.]', title = title, cbar_label=cbar_label, extent = [-180, 180, -90, 90],
                                      ...
                                      vmin = np.nanmin(plot_variable_2d), vmax = np.nanmax(plot_variable_2d), cmap = 'bwr', path = filename, **kwargs)
            if 1 in plots:
                #PLOT 1: polar plot
                #delta_lat = 40                   # range of latitudes (in degrees) to plot
                #nlat = self.shape[0]
...
class MagnetopauseShockObj(ParamObj):
    ...
    def __init__(self, run, f, coord_list, fg_b = None, **kwargs):
        ...
        self.fg_b = fg_b 
        ParamObj.__init__(self, run, f, coord_list, **kwargs)
    def make_plot_data(self, plot_data=False, ignore = 3, count = 3, plots = [1,2,3,4,5,6,7], **kwargs):
        ...
        if plot_data:
            if 0 in plots:
                #PLOT 0: 3d mesh plot of magnetopause (a bit slow, so it is not listed in plots[] by default)
                magnetopause3dk.main(run = self.run, fileIndex = self.fileIndex, save_dir = save_dir, **kwargs)
                plt.close()
            if 1 in plots:
                #PLOT 1: x vs r_cyl of magnetopause points
                title = r'{} Magnetopause position, time={}'.format(self.run.upper(), self.fileIndex)
                plt.scatter(mpos_x, mpos_rcyl, **kwargs)
                plt.xlabel(r'x [$r_E$]')
                plt.ylabel(r'$\sqrt{y^2 + z^2}$ [$r_E$]')
                plt.title(title)
            ...
            if 2 in plots:
                ...
                plt.scatter(theta_polar, mpos_r, color = 'green', **kwargs)
                plt.scatter(theta_polar_plot, f_shue_parametrized(theta_polar_plot, r_0, alpha), color = 'blue', **kwargs)
                plt.scatter(theta_polar_plot, r_fit_mp, color='red', **kwargs)
                plt.xlabel(r'$\theta$')
            ...
            if 3 in plots:
                ...
                f_shue_fit_mp_beta_prime, theta_mp, r_mp = fit_magnetopause(self.f, run = self.run, root_dir = self.root_dir, fileIndex = self.fileIndex, threshold = 1, delta = 0.05, plot = True, **kwargs)
            ...
            if 4 in plots:
                ...
                f_shue_fit_bs_n, theta_bs_n, r_bs_n = fit_bow_shock(self.f, run = self.run, root_dir = self.root_dir, fileIndex = self.fileIndex, threshold = 2, delta = 0.05, method = 'n', plot = True, **kwargs)
                f_shue_fit_bs_b, theta_bs_b, r_bs_b = fit_bow_shock(self.f, run = self.run, root_dir = self.root_dir, fileIndex = self.fileIndex, threshold = 0.96, delta = 0.01, method = 'b', plot = True, **kwargs)
                ...
                plt.plot( r_fit_mp * np.cos(theta_polar_plot), r_fit_mp * np.sin(theta_polar_plot), color='red', label = 'Magnetopause (streamline)', **kwargs )
                plt.plot( r_fit_mp_beta_prime * np.cos(theta_polar_plot), r_fit_mp_beta_prime * np.sin(theta_polar_plot), color = 'green', label = r'Magnetopause ($\beta^\prime$)', **kwargs )
                plt.plot( r_fit_bs_n * np.cos(theta_polar_plot), r_fit_bs_n * np.sin(theta_polar_plot), color = 'blue', label = r'Bow shock ($M_S$)', **kwargs )
                plt.plot( r_fit_bs_b * np.cos(theta_polar_plot), r_fit_bs_b * np.sin(theta_polar_plot), color = 'black', label = r'Bow shock (B deflection)', **kwargs )
                # plot B field vectors
                yeq0_ind = int(self.fg_b.shape[1]/2)
                B = (self.fg_b[:,:,:,0]**2 + self.fg_b[:,:,:,1]**2 + self.fg_b[:,:,:,2]**2)**0.5
                x, y, z = fg_grid(self.f, fg_b = self.fg_b)
                ...
                plt.title(title)
                plt.legend()
                filename = '{}magnetopause_bow_shock_fits_xz_{}{}.png'.format(save_dir, str(self.fileIndex).zfill(5), self.file_suffix)
                plt.savefig(filename)
                plt.close()
            if 5 in plots:
                # PLOT 5:
                ...
                plt.plot( r_fit_mp * np.cos(theta_polar_plot), r_fit_mp * np.sin(theta_polar_plot), color='red', label = 'Magnetopause (streamline)', **kwargs )
                plt.plot( r_fit_mp_beta_prime * np.cos(theta_polar_plot), r_fit_mp_beta_prime * np.sin(theta_polar_plot), color = 'green', label = r'Magnetopause ($\beta^\prime$)', **kwargs )
                plt.plot( r_fit_bs_n * np.cos(theta_polar_plot), r_fit_bs_n * np.sin(theta_polar_plot), color = 'blue', label = r'Bow shock (density)', **kwargs )
                plt.plot( r_fit_bs_b * np.cos(theta_polar_plot), r_fit_bs_b * np.sin(theta_polar_plot), color = 'black', label = r'Bow shock (B deflection)', **kwargs )
                # plot B field vectors
                ...
                plt.quiver(x2d[0::dq,0::dq]/R_EARTH, y2d[0::dq,0::dq]/R_EARTH, 
                           2*self.fg_b[0::dq,0::dq,zeq0_ind,0]/B[0::dq,0::dq,zeq0_ind], 2*self.fg_b[0::dq,0::dq,zeq0_ind,1]/B[0::dq,0::dq,zeq0_ind], **kwargs )
                plt.legend()
                plt.xlim( [f.read_parameter('xmin')/R_EARTH, f.read_parameter('xmax')/R_EARTH] )
                plt.ylim( [f.read_parameter('ymin')/R_EARTH, f.read_parameter('ymax')/R_EARTH] )
                filename = '{}magnetopause_bow_shock_fits_xy_{}{}.png'.format(save_dir, str(self.fileIndex).zfill(5), self.file_suffix)
                plt.savefig(filename)
                plt.close()
            if 6 in plots:
                ...
                theta_shue = np.arange( -(np.pi-step), np.pi-step, step)
                r_shue, r_0_shue, alpha_shue = f_shue(theta_shue, run = self.run)
                plt.plot( r_shue * np.cos(theta_shue), r_shue * np.sin(theta_shue), color = 'black', label = 'Shue (1997)' , **kwargs)
                # plot B field vectors
                yeq0_ind = int(self.fg_b.shape[1]/2)
                B = (self.fg_b[:,:,:,0]**2 + self.fg_b[:,:,:,1]**2 + self.fg_b[:,:,:,2]**2)**0.5
                x, y, z = fg_grid(self.f, fg_b = self.fg_b)
                ...
                plt.title(title)
                plt.legend()
                filename = '{}magnetopause_bow_shock_shue_xy_{}{}.png'.format(save_dir, str(self.fileIndex).zfill(5), self.file_suffix)
                plt.savefig(filename)
                plt.close()
            if 7 in plots:
                ...
                r, theta, phi = self.coord_list_sphere
                self.make_lonlat_plot(phi, theta, plot_variable_2d, xlabel='longitude [deg.]', ylabel='latitude [deg.]', title = title, cbar_label=cbar_label, extent = [-180, 180, -90, 90],
                                      vmin = 0, vmax = 100, path = filename, **kwargs)
            return plt.gca()        
...
def analyze_data(fileIndex_list):
    ...
    for fileIndex in fileIndex_list:
        ...
        # NOTE on coordinates:
        # r is radius [meters]
        # latitude is referred to by 'lat'        -pi/2<lat<pi/2  [in radians]
        # co-latitude is referred to by 'theta'   0<theta<pi
        # phi is ~longitude                       -pi<phi<pi
        ...
        if ('1' in varnames) or ('2' in varnames) or ('3' in varnames) or ('4' in varnames) or ('5' in varnames):
            ...
            trace_method = 'integrateB'            #options: 'dipole', 'integrateB'
              # input file name to enable @sidecar (pickle the results and save in sidecar directory)
            ...
            if '1' in varnames:
                ...
                fOO = FieldOpenObj(run, f, coord_list_aurora, fileIndex = fileIndex, fg_b = fg_b, max_iterations = 4000, dx = R_EARTH / 10, make_plot_data = True, plot_data=ARGS.plot, file_suffix = ARGS.suffix)
                save_list.append(fOO)
            if '2' in varnames:
                # 2. proton differential number flux
                #pFO = pFluxObj(run, f, coord_list_magnetosphere, make_plot_data = True )
                #if run != 'EGP':     # EGP doesn't have particle flux data before t=352
                pFO_traced = pFluxObj(run, f, coord_list_aurora, fileIndex = fileIndex, coord_list_traced = coord_list_traced, make_plot_data = True, plot_data=ARGS.plot, file_suffix = ARGS.suffix)
            ...
            if '3' in varnames:
                ...
                print('hello!')
                JO_traced = FAC_Obj(run, f, coord_list_aurora, fileIndex = fileIndex, coord_list_traced = coord_list_traced,fg_b = fg_b, make_plot_data = True, plot_data=ARGS.plot, file_suffix = ARGS.suffix)
                JO_traced.make_plot_data(style = 'jpar', b_scaled = True)
                save_list.append(JO_traced)
            if '4' in varnames:
                # 4. Magnetopause position
                MSO = MagnetopauseShockObj(run, f, coord_list_aurora, fileIndex = fileIndex, fg_b=fg_b, make_plot_data = True, plot_data=ARGS.plot, file_suffix = ARGS.suffix)
            ...
            if '5' in varnames:
                ...
                dBO = dBdtObj(run, f, coord_list_magnetosphere, fileIndex = fileIndex,  make_plot_data = True, file_suffix = '_5re'+ARGS.suffix, plot_data=ARGS.plot)
                save_list.append(dBO)
            if ARGS.save == True:
                # Save data into a .csv file
                save_obj = ParamObj(run, f, coord_list_aurora, coord_list_traced = coord_list_traced, file_suffix = ARGS.suffix)
    ...
    #            pt.plot.plot_colormap3dslice(filename=filename,var='proton/vg_rho',normal='y', boxre=[-15,15,-15,15],  vmin = 1e5, vmax = 1e8,run=run,colormap='plasma',step=fileIndex,outputdir=save_dir,outputfile='vg_rho_gradpe_lines_yeq0_tail_{}_{}.png'.format(run,fileIndex),Earth=1, streamlines = 'vg_e_gradpe',streamlinecolor='black',streamlinedensity=2,streamlinethick=0.8, cutpointre=0)
            if run != 'EGI':
                pt.plot.plot_colormap3dslice(filename=filename,var='vg_e_parallel',normal='y', boxre=[-20,0,-15,15], vmin = -2e-3, vmax = 2e-3, run=run,colormap='bwr',step=fileIndex,outputdir=save_dir,outputfile='vg_e_parallel_yeq0_tail_{}_{}{}.png'.format(run,fileIndex,ARGS.suffix),Earth=1, streamlines = 'vg_b_vol',streamlinecolor='black',streamlinedensity=2,streamlinethick=0.8, cutpointre=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-latmin', default = -90, help="minimum latitude, (range -90 to 90)" ) # note lat = -lat_input * (pi/180) 
    parser.add_argument('-latmax', default = 90, help="maximum latitude, (range -90 to 90)" )
    ...
    parser.add_argument('-suffix', default='', help="set this flag to append string to all files, e.g. _north or _south")  #default: append=False when -save flag not set
    ...
    ARGS.savefile = ARGS.savefile + ARGS.suffix
...
#    # Frame extent for this job given as command-line arguments
    if ARGS.startstop is None:
        ...
        fileIndex_list = range(int(ARGS.startstop[0])*delta_nframes, (int(ARGS.startstop[1]))*delta_nframes + 1, delta_nframes)
