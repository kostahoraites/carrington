...
if args.timeseries:
    for i in range(npos):
        ...
        for j, key in enumerate(keys):
            ...
            axes[j].set_ylim([float(args.vmin), float(args.vmax)])
        axes[npos-1].xlabel = 'time [seconds]'
...
def keo(key):
    ...
    if args.dl:
        ...
        keogram.keogram(l, t[:,0], dct_plot[key], filename=filename, log = args.log, shading='auto', cmap = args.cmap,
                        ...
                        ylabel ='time [sec]', title = label + ', ' + args.run.upper(), cbar_label = label, vmin = float(args.vmin), vmax = float(args.vmax))   
    else:
        ...
        keogram.keogram(r1[0,0:], t[:,0], dct_plot[key], filename=filename, log = args.log, shading='auto', cmap = args.cmap,
                        ...
                        ylabel ='time [sec]', title = label + ', ' + args.run.upper(), cbar_label = label, xlim = args.xlim, ylim = args.ylim, vmin = float(args.vmin), vmax = float(args.vmax))
