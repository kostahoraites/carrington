...
if args.filein is None:
    dct, nvars = read_vsc_data('virtual_sc_data_{}.txt'.format(args.run.lower()), n_sc = n_sc_input, tvar=args.t)
else:
    ...
    for j, key in enumerate(keys):
        try:
            ...
            if args.filter:
                ...
                if args.norm:
                    dct_plot[key][:,i] = value[:,i] / np.nanmax(np.abs(value[:,i]))
...
if args.timeseries:
    ...
    #fig.tight_layout()
    filename = '{}{}_{}_coords_{}_{}_{}.png'.format(save_dir, fileprefix + '/' + fileprefix, args.run.upper(), r1[0,i], r2[0,i], r3[0,i])
    mkdir_path(filename)
...
if args.keogram:
    for j, key in enumerate(keys):
    filename = '{}{}_keogram_{}_{}.png'.format(save_dir, fileprefix + '/' + fileprefix, key, args.run.upper())
    mkdir_path(filename)
    value = dct_plot[key]
    if args.filter:
        label = r"$\Delta$ " + key.replace('_', '\\_')
...
if args.heatmap:
    ...
    for i in range(ntime):
        filename = '{}{}/{}_heatmap_{}_{}_{}.png'.format(save_dir, key, fileprefix + '/' + fileprefix, key, args.run.upper(), str(int(t[i,0])).zfill(5))
        mkdir_path(filename)
