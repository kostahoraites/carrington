...
def get_bulk(run, fileIndex): #fetch bulk file name

    #fileLocation="/wrk/group/spacephysics/vlasiator/3D/EGE/bulk/"
    #fileN = "bulk.0002193.vlsv"
    #return (fileLocation+fileN)
    if run == 'EGL':
        return "/wrk/group/spacephysics/vlasiator/3D/{}/bulk/bulk1.{}.{}.vlsv".format(run.upper(), run.lower(), str(fileIndex).zfill(7) )
    elif run == 'EGM':
        return "/wrk/group/spacephysics/vlasiator/3D/{}/bulk/bulk.{}.vlsv".format(run.upper(), str(fileIndex).zfill(7) )
    elif run == 'EGN':
        return "/wrk/group/spacephysics/vlasiator/3D/{}/bulk1.{}.vlsv".format(run.upper(), str(fileIndex).zfill(7) )
    elif run == 'EGO':
        return "/wrk/group/spacephysics/vlasiator/3D/{}/bulk1.{}.vlsv".format(run.upper(), str(fileIndex).zfill(7) )
    #return "/wrk/group/spacephysics/vlasiator/3D/{}/bulk/bulk1.{}.{}.vlsv".format(run.upper(), run.lower(), str(fileIndex).zfill(7) )
...
def get_poynting_vectors(coordinate_points): #TODO
    ...
    #name = get_bulk(run, fileIndex)
    name = get_vlsvfile_fullpath(run, fileIndex)
...
def make_streamlines(run, fileIndex):
    ...
    yt_dataset = yt.loaders.load_uniform_grid(
        data,
        sizesfs,
    ...
    streamlines_pos = yt.visualization.api.Streamlines(yt_dataset, streamline_seeds,
                                                    ...
                                                    #"Vx", "Vy", "Vz", length=240, direction=1)
...
def get_magnetopause(streams, ignore, count):
    ...
    x_points = np.array(np.arange(5, -30, -0.5))
    #x_points = np.array(np.arange(5, -60, -0.5))
...
def main(run = 'EGL', fileIndex = 1760, save_dir = './'):
    '''
    ...
    streams = make_streamlines(run, fileIndex)
    ...
    plt.savefig('{}magnetopause3d_{}_{}.png'.format(save_dir, run, fileIndex))
