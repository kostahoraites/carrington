...
#global CELLSIZE
global ROOT_DIR
ROOT_DIR = '/wrk-vakka/users/emilirin/carrington/'
global mu_0
mu_0 = 4e-7 * np.pi
...
def volume_shell(data, x, y, z, threshold, delta, step = 0.5): #STEP = 1 OR STEP = 0.5
...
def fit_magnetopause(f, run = '', root_dir = '', fileIndex = '', threshold = 1, delta = 0.05, plot = True):
    ...
    if True:
        ...
        m_x = r*np.cos(theta)
        m_R = r*np.sin(theta)
        ax.plot(m_x, m_R, color='cyan', linewidth=2, zorder=1)
        ...
        pt.plot.plot_colormap3dslice(filename=bulkLocation+bulkname1,var='proton/vg_beta_star', boxre=[-15, 15, -15, 15], normal =normal, run=run,
                                     colormap='seismic',vmin=10**(-1),vmax=10,step=j,outputdir=outputLocation,
                                     outputfile='test_colormap_3dslice_beta_star_{}eq0_{}_{}.png'.format(normal, run, j), 
                                     Earth=1, streamlines=streamlines, streamlinedensity=sl_density, cutpointre=0, axes = ax, scale=scale)
        ...
        ax.plot(model(polyline), polyline, color='lime', linewidth=2, zorder=3)
        ...
        ax.plot(mp_nose_x, mp_nose_y, marker='o', mfc='lime', mec='lime', zorder=4)
        
        #----------------------------------------------------------------------------
        filename = '{}beta_star_colormap_xy_{}_step0.5.pdf'.format(save_dir, str(fileIndex).zfill(5))
